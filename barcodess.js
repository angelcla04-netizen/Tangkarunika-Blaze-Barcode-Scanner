...
const products = {
  ...
  "1834561": { name: "Sumbi's Accessories", price: 8000 },
...
function removeOne(barcode) {
  if (!cart[barcode]) return;
  cart[barcode].quantity -= 1;
  if (cart[barcode].quantity <= 0) delete cart[barcode];
  updateCartUI();
}
...
function handlePay() {
  ...
  if (total <= 0) {
    flashWarning("Cart is empty.");
    return;
  }
  if (isNaN(cash) || cash < total) {
    document.getElementById("changeDisplay").textContent = fmt(0);
    flashWarning("Cash is less than total. Please enter enough.");
    return;
  }
...
function renderReceipt(cash, change) {
  ...
  const lines = Object.values(cart).map(
    (i) => `<div>${i.name} x${i.quantity} @ ${fmt(i.price)} = <strong>${fmt(i.price * i.quantity)}</strong></div>`
  );
...
function completeSale() {
  ...
  if (total <= 0) {
    flashWarning("Add items before completing the sale.");
    return;
  }
  if (cashTxt === "Rp 0" || changeTxt === "Rp 0") {
    flashWarning("Press Pay to calculate cash and change first.");
    return;
  }
...
function clearPaymentUI() {
  ...
  document.getElementById("cashGivenDisplay").textContent = fmt(0);
  document.getElementById("changeDisplay").textContent = fmt(0);
}
...
function renderHistory() {
  ...
  wrap.innerHTML = "";
  if (history.length === 0) {
    wrap.innerHTML = "<p>No receipts yet.</p>";
    return;
  }
  history.slice().reverse().forEach((rec) => {
    const div = document.createElement("div");
    const itemsHtml = Object.values(rec.items)
...
function clearHistory() {
  localStorage.removeItem("il_receipts");
  renderHistory();
}
...
function exportCurrentReceiptCsv() {
  const total = cartTotal();
  if (total <= 0) {
    flashWarning("No items to export.");
    return;
  }
  ...
  Object.values(cart).forEach((i) => {
    csv += `${i.name},${i.quantity},${i.price},${i.price * i.quantity}\n`;
  });
  csv += `Subtotal,, ,${total}\n`;
  csv += `Cash,, ,${cash}\n`;
  csv += `Change,, ,${change}\n`;
  ...
  a.href = url;
  a.download = `IL_Receipt_${Date.now()}.csv`;
    a.click();
...
function startScanner() {
  ...
  Quagga.init(
    ...
    function (err) {
      ...
      Quagga.start();
      scannerRunning = true;
      document.getElementById("toggleScannerBtn").textContent = "✖ Close Camera";
    }
  );
}
...
function stopScanner() {
  ...
  document.getElementById("toggleScannerBtn").textContent = "📷 Open Camera";
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("floatingScannerBtn").addEventListener("click", () => {
    if (scannerRunning) stopScanner();
    else startScanner();
  });
...
document.getElementById("captureBtn").addEventListener("click", () => {
  if (scannerRunning) {
    ...
    flashWarning("Camera is not open.");
  }
});